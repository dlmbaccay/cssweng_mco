...
function PetProfilePage() {
  ...
  const openEdit = () => {
    if (currentUser && currentUserID === petOwnerID) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (selectedProfileFile) {
        await uploadFile(
          storage.ref(`petProfilePictures/${petID}/profilePic`),
          selectedProfileFile,
          "photoURL"
        );
      }
  ...
  const handlePetNameVal = (val) => {
    ...
    if (
      checkPetNameValue.startsWith(" ") ||
      checkPetNameValue.endsWith(" ") ||
      checkPetNameValue.includes("  ")
    ) {
      setEditedPetName(checkPetNameValue);
      setEditPetNameValid(false);
    } else if (
      checkPetNameValue.length >= 3 &&
      checkPetNameValue.length <= 15
    ) {
      setEditedPetName(checkPetNameValue);
  ...
  return (
    <div id="root" className="flex">
      <div className="w-fit md:flex hidden">
        {petOwnerPhotoURL && petOwnerUsername && (
          <ExpandedNavBar
            props={{
      ...
      {pet && currentUser && (
        <div className="h-screen w-full">
          ...
          <div
            id="header-container"
            className="h-1/5 border-l border-neutral-300 hidden md:block"
          >
            <CoverPhoto
              src={petOwnerCoverPhotoURL}
              alt={petOwnerUsername + " cover photo"}
            />
          </div>
          ...
          <div id="content-container" className="h-full md:h-4/5 flex flex-row">
            ...
            <div className="fixed hidden lg:flex flex-col lg:w-80 h-4/5 bg-snow border border-neutral-300 justify-start items-center">
              ...
              <div className="text-center mt-24 w-80">
                ...
                <Link
                  ...
                  className="hover:text-grass text-md text-raisin_black hover:font-bold transition-all"
                >
                  @{petOwnerUsername}
                </Link>
              </div>
                  ...
                  >
                        ...
                        <div className="w-full pl-8 md:pl-0 pr-8">
                          ...
                          <div className="mb-4 w-full">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setEditedPetHobbies(e.target.value)
                              }
                              required
                            />
                          </div>
                          ...
                          <div className="mb-4 w-full">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setEditedPetFavoriteFood(e.target.value)
                              }
                              required
                            />
                          </div>
                    ...
                    <div className="mt-4 pb-8 md:pb-0 flex justify-end gap-4">
                      ...
                      <button
                        ...
                        className={`w-20 h-10 font-semibold rounded-md bg-xanthous text-white transition-all ${
                          editPetNameValid ? "hover:bg-pistachio" : "opacity-50"
                        }`}
                      >
                        Save
                      </button>
              ...
              <div className="text-center mt-6 flex flex-row gap-10 w-80 items-center justify-center">
                <div className="flex flex-col items-center">
                  <span className="text-raisin_black font-bold">
                    {" "}
                    {pet.followers ? pet.followers.length : 0}
                  </span>
                  <span className="text-grass font-bold text-sm">
                    Followers
                  </span>
                </div>
              </div>
              ...
              <div className="mt-4 mb-2 flex flex-col items-center w-full gap-2">
                {hidden && !hidden.includes("breed") ? (
                  <div id="icons" className="group flex flex-row gap-2 items-center">
                    <i className="fa-solid fa-bread-slice "></i>
                    <span class="group-hover:opacity-100 transition-opacity text-white bg-black px-1 text-sm text-gray-100 rounded-md absolute left-1/2 -translate-x-1/2 -translate-y-full opacity-0 m-4 mx-auto">Breed</span>
                    <p>{breed}</p>
                  </div>
                ) : (
                  ""
                ...
                {hidden && !hidden.includes("sex") ? (
                  <div id="icons" className="group flex flex-row gap-2 items-center">
                    ...
                    <span class="group-hover:opacity-100 transition-opacity text-white bg-black px-1 text-sm text-gray-100 rounded-md absolute left-1/2 -translate-x-1/2 -translate-y-full opacity-0 m-4 mx-auto">Sex</span>
                    <p>{sex}</p>
                  </div>
                ) : (
                  ""
                ...
                {hidden && !hidden.includes("birthplace") ? (
                  <div id="icons" className="group flex flex-row gap-2 items-center">
                    ...
                    <span class="group-hover:opacity-100 transition-opacity text-white bg-black px-1 text-sm text-gray-100 rounded-md absolute left-1/2 -translate-x-1/2 -translate-y-full opacity-0 m-4 mx-auto">Birthplace</span>
                    <p>{birthPlace}</p>
                  </div>
                ) : (
                  ""
                ...
                {hidden && !hidden.includes("favoriteFood") ? (
                  <div id="icons" className="group flex flex-row gap-2 items-center">
                    ...
                    <span class="group-hover:opacity-100 transition-opacity text-white bg-black px-1 text-sm text-gray-100 rounded-md absolute left-1/2 -translate-x-1/2 -translate-y-full opacity-0 m-4 mx-auto">Favorite Food</span>
                    <p>{favoriteFood}</p>
                  </div>
                ) : (
                  ""
                ...
                {hidden && !hidden.includes("hobbies") ? (
                  <div id="icons" className="group flex flex-row gap-2 items-center">
                    ...
                    <span class="group-hover:opacity-100 transition-opacity text-white bg-black px-1 text-sm text-gray-100 rounded-md absolute left-1/2 -translate-x-1/2 -translate-y-full opacity-0 m-4 mx-auto">Hobbies</span>
                    <p>{hobbies}</p>
                  </div>
                ) : (
                  ""
            ...
            <div
              ...
              className="overflow-hidden flex flex-col lg:translate-x-80 lg:w-[calc(100%-20rem)] w-full"
            >
              <div
                id="flex-profile-details"
                className="lg:hidden w-full h-24 md:h-12 bg-snow flex flex-row items-center md:pl-10 gap-4 md:justify-start justify-center"
              >
                <Image
                  src={petPhotoURL}
                  alt={petName + " profile picture"}
              ...
              <div
                id="tab-actions"
                className="flex flex-row h-12 font-shining bg-snow divide-x divide-neutral-300 border-b border-t border-neutral-300 drop-shadow-sm md:justify-start justify-between"
              >
                <button
                  className={`w-1/2 md:w-fit text-sm md:text-base md:px-14 py-2 text-raisin_black hover:bg-citron hover:text-white focus:outline-none ${
              ...
              <div
                id="tab-container"
                className="overflow-y-scroll h-full w-fit md:w-full bg-[#FAFAFA]"
              >
                {/* Tagged Posts */}
                    ...
                    {taggedPosts.length === 0 && (
                      <div className="flex mt-8 mb-20 md:mb-8 flex-col gap-8 justify-start items-center">
                        ...
                        <div className="flex flex-col items-center justify-center h-full w-full">
                          ...
                          <div className="mt-2 font-bold text-grass text-sm md:text-base">
                ...
                {activeTab === "Milestones" && (
                  <div className="flex flex-col mt-8 mb-8 gap-8 items-center">
                    ...
                    {taggedPosts.filter(
                      (post) => post.postCategory === "Milestones"
                    ).length === 0 && (
                    ...
                    {taggedPosts
                      .filter((post) => post.postCategory === "Milestones")
                      .map((post, index) => (
                        <div key={post.id}>
                          <PostSnippet
                            props={{
...
function PetNameMessage({ petName, petNameValid, loading }) {
  if (loading) {
    ...
    String(petName).length < 3 &&
    String(petName).length > 15 &&
    !petNameValid
  ) {
    ...
    (String(petName).startsWith(" ") || String(petName).endsWith(" ")) &&
    !petNameValid
  ) {
    ...
    return (
      <p className="mt-2 ml-2">
        Only periods and underscores allowed for special characters.
      </p>
    );
  }
}
